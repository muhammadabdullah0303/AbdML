...
class AbdBase:
    ...
    def Train_ML(self, params, model_name, e_stop=50):
        ...
        if self.X_test is not None:
            final_test_preds = np.mean(test_preds, axis=1) if self.num_classes > 2 else np.mean(test_preds, axis=1)
        
        print(f"Train Scores: {train_scores}")
        print(f"Out-of-Fold Scores: {oof_scores}")
        
        return oof_predictions, final_test_preds if self.X_test is not None else None
