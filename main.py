...
class AbdBase:
    ...
    def CAT(self, params):
        ...
        for fold, (train_idx, val_idx) in enumerate(tqdm(kfold.split(self.X_train, self.y_train), desc="Training Folds", total=self.n_splits)):
            ...
            test_preds[:, fold] = model.predict_proba(test_pool)[:, 1] if self.problem_type == 'classification' else model.predict(test_pool)
        ...
        mean_test_preds = np.round(test_preds.mean(axis=1)).astype(int) if self.problem_type == 'classification' else test_preds.mean(axis=1)
        return oof_predictions, mean_test_preds
