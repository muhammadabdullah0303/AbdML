...
#                  n_splits=5, cat_features=None, num_classes=None):
...
#                 y_train_pred = model.predict_proba(X_train) if self.num_classes > 2 else model.predict_proba(X_train)[:, 1]
#                 y_val_pred = model.predict_proba(X_val) if self.num_classes > 2 else model.predict_proba(X_val)[:, 1]
#             else:
...
#                 test_preds[:, fold] = model.predict_proba(self.X_test) if self.num_classes > 2 else model.predict_proba(self.X_test)[:, 1]
...
class AbdBase:
    ...
    def CAT(self, params):
        ...
        print(f"Overall Train {self.metric.upper()}: {np.mean(train_scores):.5f}")
        print(f"Overall OOF {self.metric.upper()}: {np.mean(oof_scores):.5f}")
