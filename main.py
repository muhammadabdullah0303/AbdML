...
class AbdBase:
    ...
    def Train_ML(self, params, model_name, e_stop=50):
        ...
        for fold, (train_idx, val_idx) in enumerate(tqdm(kfold.split(self.X_train, self.y_train), desc="Training Folds", total=self.n_splits)):
            ...
            if model_name == 'LGBM':
                model.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=e_stop if self.early_stop else None)
            elif model_name == 'CAT':
        ...
        print(f'Mean Train Score: {mean_train_score:.4f}, Mean OOF Score: {mean_oof_score:.4f}')

        if self.test_prob and self.X_test is not None:
            test_preds_mean = np.mean(test_preds, axis=1)
            return mean_train_score, mean_oof_score, test_preds_mean
        return mean_train_score, mean_oof_score, oof_predictions
